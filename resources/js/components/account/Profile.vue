    ...
    >
      ...
      >
        <div class="p-4 md:p-12 text-center lg:text-left">
          ...
          <DxButton text="Quay lại" @click="backToAccountPage" class="back btn btn-warning"/>
          <div
          ...
          <h1 class="text-3xl font-bold pt-8 lg:pt-0">{{ profile.name }} - {{ profile.email }}</h1>
          <div
            class="mx-auto lg:mx-0 w-4/5 pt-3 border-b-2 border-green-500 opacity-25"
          ></div>
          <!-- <p class="pt-4 text-base font-bold flex items-center justify-center lg:justify-start">
                    <span class="h-4 fill-current text-green-700 pr-4"> Ngày sinh</span>
                    What you do
                </p>
                <p class="pt-4 text-base font-bold flex items-center justify-center lg:justify-start">
                    <span class="h-4 fill-current text-green-700 pr-4">Giới tính</span>
                    What you do
                </p> -->
          <div class="row">
            <div
              class="col-4 font-bold mt-2 h-4 fill-current text-green-700 pr-4"
            >
              Ngày sinh
            </div>
            <div class="col-8 mt-2">
              <DxDateBox v-model="birthday" type="date" />
            </div>
          </div>
          <div class="row">
            <div
            ...
            <div class="col-8">
              <DxRadioGroup :items="genderItems" v-model="gender" />
            </div>
          </div>
          <div class="row">
            <div
          ...
          <div class="pt-12 pb-8">
            <DxButton :text="updateText" @click="updateProfile" />
          </div>
...
export default {
  ...
  mounted() {
    console.log("mouted");
    if(location.search.substring(1) === 'isAdmin=false'){
      alert('Bạn không phải là admin.\n Vào bảng user để set isAdmin = true')
    }
    setTimeout(() => {
      this.birthday=this.profile.birthday ? new Date(this.profile.birthday) : null;
    }, 500);
  },
  methods: {
    updateProfile() {
    ...
    this.updateText = 'Đang cập nhật ...';
      window.axios.put("/api/account/profile/update", {
        ...
        birthday: this.getDate(this.birthday),
      }).then(res=>{
          this.updateText = 'Cập nhật';
          if(res.data){
              alert('Cập nhật thành công')
          }
          else{
              alert('Cập nhật thất bai')
              location.reload();
          }
      });
    },
